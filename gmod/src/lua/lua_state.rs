...
use crate::{lua::*, userdata::TaggedUserData};

pub type LuaCStr = &'static std::ffi::CStr;
...
impl LuaState {
    ...
    #[inline(always)]
    pub unsafe fn get_field(&self, index: i32, k: LuaCStr) {
        (LUA_SHARED.lua_getfield)(*self, index, k.as_ptr())
    }
    ...
    #[inline(always)]
    pub unsafe fn set_field(&self, index: i32, k: LuaCStr) {
        (LUA_SHARED.lua_setfield)(*self, index, k.as_ptr())
    }

    #[inline(always)]
    pub unsafe fn get_global(&self, name: LuaCStr) {
        (LUA_SHARED.lua_getfield)(*self, LUA_GLOBALSINDEX, name.as_ptr())
    }

    #[inline(always)]
    pub unsafe fn set_global(&self, name: LuaCStr) {
        (LUA_SHARED.lua_setfield)(*self, LUA_GLOBALSINDEX, name.as_ptr())
    }
    ...
    #[inline(always)]
    pub unsafe fn check_userdata(&self, arg: i32, name: LuaCStr) -> *mut TaggedUserData {
        (LUA_SHARED.lual_checkudata)(*self, arg, name.as_ptr()) as *mut _
    }
    ...
    #[inline(always)]
    pub unsafe fn new_metatable(&self, name: LuaCStr) -> bool {
        (LUA_SHARED.lual_newmetatable)(*self, name.as_ptr()) == 0
    }

    pub unsafe fn new_userdata<T: Sized>(&self, data: T, metatable: Option<i32>) -> *mut T {
        let has_metatable = if let Some(metatable) = metatable {
    ...
    pub unsafe fn debug_getinfo_from_ar(&self, ar: &mut LuaDebug, what: LuaCStr) -> Result<(), ()> {
    ...
    pub unsafe fn debug_getinfo_at(&self, level: i32, what: LuaCStr) -> Option<LuaDebug> {
        let mut ar = MaybeUninit::uninit();
        if (LUA_SHARED.lua_getstack)(*self, level, ar.as_mut_ptr()) != 0
