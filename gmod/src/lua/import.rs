...
use super::{returns::ValuesReturned, LuaDebug, LuaError, State as LuaState};
...
pub struct LuaSharedInterface(
    ...
    #[cfg(debug_assertions)] UnsafeCell<*mut std::thread::ThreadId>,
);
impl LuaSharedInterface {
    ...
    pub(crate) fn debug_assertions(&self) {
        ...
        assert_eq!(
			thread::current().id(),
			unsafe{* { *self.1.get() }},
			"The Lua state is being accessed from a different thread than it was initialized on. This is not allowed."
		);
    }
...
pub static mut LUA_SHARED: LuaSharedInterface = LuaSharedInterface(
